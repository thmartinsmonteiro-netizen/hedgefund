...
def load():
    ...
    if "multiplier" not in pos.columns:
        pos["multiplier"] = 1
    pos["multiplier"] = pd.to_numeric(pos["multiplier"], errors="coerce").fillna(1)
    if "instrument" not in pos.columns:
        pos["instrument"] = "equity"
...
def trade_costs(tr, a):
    tr = tr.copy()
    if "multiplier" not in tr.columns:
        tr["multiplier"] = 1
    tr["multiplier"] = pd.to_numeric(tr["multiplier"], errors="coerce").fillna(1)
    tr["gross_usd"] = tr["qty"] * tr["price_native"] * tr["fx_to_usd"] * tr["multiplier"]
...
def build_nav():
    ...
    for d in dates:
        ...
        for _, p in pos.iterrows():
            ...
            mv_total += qty * close * fx * float(p["multiplier"])
...
def positions_latest(a, pos, tr, px, nav):
    ...
    for _, p in pos.iterrows():
        ...
        mv = None if (qty == 0 or close is None or fx is None) else qty * close * fx * float(p["multiplier"])
        out.append({"ticker": p["ticker"], "name": p["name"], "instrument": p["instrument"], "status": p["status"], "sleeve": p["sleeve"],
...
def limit_checks(a, plat, latest_nav):
    ...
    msgs = []
    if not held.empty:
        w = (held["cost_basis_usd"].abs() / latest_nav).max()
        msgs.append(("Largest position at cost", f"{w:.1%}", "BREACH" if w > L["max_position_at_cost"] else "ok"))
        mv = held["mv_usd"].fillna(0)
        gross, net = mv.abs().sum() / latest_nav, mv.sum() / latest_nav
        msgs.append(("Gross exposure % NAV", f"{gross:.0%}", "BREACH" if gross > L.get("max_gross_exposure", 1.5) else "ok"))
        msgs.append(("Net exposure % NAV", f"{net:.0%}", "BREACH" if not (L.get("min_net_exposure", -0.5) <= net <= L.get("max_net_exposure", 1.2)) else "ok"))
        opt = held[held["instrument"].isin(["option"])]
        if not opt.empty:
            prem = (opt.loc[opt["qty"] > 0, "cost_basis_usd"].sum()) / latest_nav
            msgs.append(("Long option premium at risk % NAV", f"{prem:.1%}", "BREACH" if prem > L.get("max_option_premium_at_risk", 0.05) else "ok"))
